...
def save_population(population,gen, selection_method) : 
    
    root_path = "../Logs/MyTest/"+selection_method
    try:
...
def simple_EA(target, number_of_generation, mut_probs, init_pop,k, selection_method, log_folder) : 
    ...
    save_population(prev_population,0,log_folder+selection_method)
    ...
    while (n > 0) and (maxfitness<1)   :
        ...
        maxfitness = max([ind.fitness for ind in prev_population])
        n -=1
        save_population(prev_population, number_of_generation - n, log_folder+selection_method)
